...
<head>
  ...
  <title>ผลการเบิกจ่ายงบประมาณ กองโรคจากการประกอบอาชีพและสิ่งแวดล้อม</title>
...
<body>
   ...
   <header id="header" class="header fixed-top d-flex ">
    ...
    <section class="section">
        <div class="pagetitle" style ="color:#f0f4fb">
          <h1>Chart</h1>
          <nav>
              <ol class="breadcrumb">
                          ...
                          <div class="row">
                              <div class="col-4"  >
                                <div class="row" style ="background-color:#00BFFF;text-align:center; font-size: 16px;color:black;font-weight: bold;" >
                                  ...
                                  <div  >งบประมาณ</div>
                                  <?php
                                      $sql_total = "SELECT sum(amount) as total FROM tb_receive WHERE type_budget IN('งบตามแผน') ";
                                      ...
                                      while ($sumtotal = mysqli_fetch_array($total)) {
                                          ...
                                          $p = number_format($sumtotal["total"]);
                                      } 
                                  ?>
                                  <div><?php echo $p ?></div>
                                </div>
                                <div class="row"  style ="background-color:#16E2F5;text-align:center; font-size: 16px;color:black;font-weight: bold;" >
                                  ...
                                  <div>งบที่ได้รับ</div>
                                  <?php
                                      $sql_total = "SELECT SUM(amount) as amount FROM tb_receive WHERE type_budget IN ('ขอรับเงินกรม', 'งวดที่ 1', 'งวดที่ 2', 'งวดที่ 2 เพิ่มเติม', 'งวดที่ 3', 'งวดที่ 4')";
                                      // $sql_total = "SELECT SUM(tb_receive.amount) as amount FROM tb_receive INNER JOIN tb_ref ON tb_receive.id_ref = tb_ref.id_ref WHERE tb_receive.type_budget LIKE 'งวด%' AND tb_ref.budget1 ='2.งบดำเนินงาน' AND tb_ref.budget2 != '2.2 งบวัสดุวิทย์' AND tb_ref.budget2 != '2.3 งบขั้นต่ำ';";
                              ...
                              <div class="col-4" style ="text-align:center; color:black;background-color:#66ff99;font-weight: bold;" >
                                  ...
                                  <?php
                                      $sql_total = "SELECT SUM(amount) as amount FROM `tb_manage` WHERE type_buget IN ('เบิกจ่าย') ";
                            ...
                            var data_p = {
                              labels: <?php echo json_encode($labels)?>, //ชื่อผลผลิต axis x
                                  datasets: [{ 
                                    ...
                                    data:<?php echo json_encode($dq3)?>,// [933162,0790553,11500,8700,412808,316786]
                                    // pointStyle: 'rect'
                                    },
                                    {
                                    label: "เบิกจ่าย Q4",
                                    ...
                                    data:<?php echo json_encode($dq4)?>,//[230765,0,208708,0,0,75966,54939]
                                    // pointStyle: 'rect'
                                    },
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <?php
                            ...
                            $sql_dq3 ="SELECT name_initials ,sum(amount) AS amount, CASE WHEN name_initials = 'กลุ่มบริหารทั่วไป' THEN 1 WHEN name_initials = 'กลุ่มนวัตกรรมฯ' THEN 2 WHEN name_initials = 'กลุ่มยุทธ์ฯ' THEN 3 WHEN name_initials = 'กลุ่มอาชีวอนามัยฯ' THEN 4 WHEN name_initials = 'กลุ่มกฎหมายฯ' THEN 5 WHEN name_initials = 'กลุ่มข้อมูลฯ' THEN 6 WHEN name_initials = 'กลุ่มสิ่งแวดล้อมฯ' THEN 7 WHEN name_initials = 'ศูนย์อ้างอิงฯ' THEN 8 WHEN name_initials = 'ศูนย์ระยอง' THEN 9 END AS n  FROM (SELECT tb_group.name_initials,SUM(tb_manage.amount) as amount FROM tb_manage INNER JOIN tb_group ON tb_manage.id_group = tb_group.id_group WHERE tb_manage.type_buget ='เบิกจ่าย' AND tb_manage.quarter = 'Q3' GROUP BY tb_group.name_initials UNION ALL SELECT name_initials as name_initials, 0 as amount FROM tb_group) as a GROUP BY name_initials ORDER BY n ASC;";
